...
(* Tokens produced by the lambda-calculus lexer. *)
type lambda_token =
  | Lambda_RParen
  | Lambda_LParen
  | Lambda_Lambda
...
(* Tokens produced by the Boolean-expression lexer. *)
type engl_token =
  | Engl_RParen
  | Engl_LParen
  | Engl_If
...
(* An optional binding represents a declared variable without a definition. *)
